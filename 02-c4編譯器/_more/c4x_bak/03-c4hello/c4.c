...
token_t sym[POOL_SIZE] = {
  {.tk=Id, .class=Sys, .name="printf", .type=INT, .val=PRTF}
};
  ...
  while (t.tk = *p) {
    ...
    if (t.tk == '\n') { // 換行
      ...
      while (id->name) { // 檢查符號是否存在
        if (!memcmp(id->name, t.name, p - t.name)) { t = *id; return; } // 有找到的話就傳回
        id++; // 找下一個符號
      }
...
int expr() {
  ...
  if (!t.tk) { printf("%d: unexpected eof in expression\n", line); exit(-1); } // EOF
    ...
    *++e = IMM; *++e = t.val; next();
    ...
    token_t id = t; next();
    printf("id.name=%.4s class=%d\n", id.name, id.class);
    if (t.tk == '(') { // id (args) ，這是 call
      int arg_count = 0;
      next();
    ...
    else if (id.class == Num) { *++e = IMM; *++e = id.val; t.type = INT; } // 該 id 是數值
    else {
      ...
      *++e = ((t.type = id.type) == CHAR) ? LC : LI; // LI  : load int, LC  : load char
    }
  }
  else if (t.tk == '(') { // (E) : 有括號的運算式 ...
    next();
    if (t.tk == Int || t.tk == Char) {
      int type = (t.tk == Int) ? INT : CHAR; next();
      while (t.tk == Mul) { next(); type = type + PTR; }
      if (t.tk == ')') next(); else { printf("%d: bad cast\n", line); exit(-1); }
      expr(Inc); // 處理 ++, -- 的情況
      t.type = type;
    }
    else {
